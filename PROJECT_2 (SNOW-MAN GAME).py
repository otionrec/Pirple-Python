...
def input_letters(one=False, two=False, three=False, four=False, five=False):
    ...
    if two==True:
        letter = textinput("Correct! Second Letter", first_guessed_letter)
        return letter

    if three==True:
        letter = textinput("Good! Third Letter", second_guessed_letter)
        return letter

    if four==True:
        letter = textinput("Great! Fourth guess", third_guessed_letter)
        return letter

    if five==True:
        letter = textinput("Fantastic! Last guess", fourth_guessed_letter)
...
def first_guess(secret_word, trys, no_correct_guesses, first_guessed_letter):
    ...
    if letter.casefold() == secret_word[0].casefold() and len(letter) == 1:
